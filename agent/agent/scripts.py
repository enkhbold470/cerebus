...
def run_web():
    ...
    try:
        subprocess.run(["adk", "web"], cwd=Path(__file__).parent.parent)
...
def run_cli():
    ...
    try:
        subprocess.run(["adk", "run", "."], cwd=Path(__file__).parent.parent)
    except KeyboardInterrupt:
        print("\n🛑 Agent stopped")
    except FileNotFoundError:
        print("❌ ADK not found. Install with: pip install google-adk")
...
def run_tests():
    ...
    try:
        test_script = Path(__file__).parent.parent / "test_agent.py"
...
def setup_environment():
    ...
    print(
        "2. Edit .env file and replace 'your_gemini_api_key_here' with your actual API key"
    )
